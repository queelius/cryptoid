...
def status(content_dir: Path | None, config_path: Path, verbose: bool):
    ...
    for md_file in sorted(content_dir.rglob("*.md")):
        rel = md_file.relative_to(content_dir)
        result = evaluate_cascade(md_file, content_dir)

        if result.config is None:
            # An _index.md with explicit `encrypted: false` is a cascade
            # opt-out; surface its declared groups so users can see the
            # cascade boundary. SOURCE_OPT_OUT distinguishes it from a
            # regular plain file with no cascade intent.
            if (
                result.source == SOURCE_OPT_OUT
                and md_file.name == "_index.md"
            ):
                idx_config = get_encryption_config(result.content)
        ...
        groups_info = result.config.groups or ["all"]
        state = "ENCRYPTED" if result.is_already_encrypted else "pending"

        try:
            users = resolve_users(result.config.groups, config)
        ...
        click.echo(
            f"  [{state:9s}] {rel}  "
            f"groups={groups_info} users={user_count} source={result.source}"
        )
