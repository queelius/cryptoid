...
Two-tier config: a global config at `$XDG_CONFIG_HOME/cryptoid/config.yaml` (defaulting to `~/.config/cryptoid/config.yaml`) merges with the local `.cryptoid.yaml`. Merge rule (uniform: local always wins on conflict, never additive): for `users`, the union of usernames is taken with local winning on password collision (warned); for `groups`, a same-named local group fully replaces the same-named global group, mirroring the file-level cascade rule that a file's `groups` field fully replaces inherited groups. `salt`, `content_dir`, and `admin` are scalar overrides (local wins). Resolution priority for `content_dir`: CLI flag > `CRYPTOID_CONTENT_DIR` env var > local config > global config. `_load_global_config()` is the canonical reader (called from `load_config()` plus several mutator commands like `add-user` and `remove-user` for cross-tier validation). Bootstrap a global config with `cryptoid init --global`. Inspect with `cryptoid config status` (paths) or `cryptoid config show` (contents, with `--show-passwords` to unmask).
